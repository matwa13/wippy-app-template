...
- **Task 19 — blank render on detail-page query failure (deferred, plan-prescribed gap):** `trip-detail.vue` has `<div v-if="trip">` with `v-else-if="isPending"` spinner but no `v-else` for the error/not-found case. The plan's template has the same gap. If the user navigates to a deleted trip id or the query throws, the page renders blank. Low-priority UX polish; revisit if Task 23 smoke test surfaces it.
- **Task 20 — create form's `endDate < startDate` is only caught server-side (deferred, plan-prescribed):** `:min-date="startDate ?? new Date()"` does NOT retroactively clear an already-picked `endDate` when `startDate` advances past it; the submit guard only checks `!endDate`. The server's `create_trip.lua` handler validates `end_date >= start_date` and returns 400, which surfaces via `host.toast` — so the user sees an error, no data corruption. Defer polishing until real-world users trip on it.
- **Task 19 — store-sync `watch` missing in list page (caught + fixed):** the reference pattern in `tasks.vue` writes fetched results back to the Pinia store (`watch(tasks, val => tasksStore.list = val)`); the plan's list page code omitted the equivalent for trips. Without it, the persisted `wippyPersist` list never refreshes, and `placeholderData` freezes at the first-ever snapshot. Fixed in `bf3c52d` by adding the same `watch`. If you regenerate from the plan doc, remember this correction.
...
## Post-plan architectural changes

After Phase 3 finished (last plan-driven commit: `f655164` marking Task 22 done), four follow-on commits landed on the branch that diverge from the task-by-task instructions above. They are listed here so readers know which code to trust. The current spec at `docs/specs/trip-planner.md` reflects the post-change architecture; this plan's task table is preserved as a historical execution log.

| Commit | Summary | What changed vs this plan |
|---|---|---|
| `e5accf4` | IATA codes | Added `trip_iata_resolver` agent as a 3rd research branch and a matching `save_iata_resolver` func that forwards resolved codes to `flights_linker`. Plan had `flights_linker` derive codes itself from raw city names. The synthesizer's `exit_func_id` hook was still present at this point. |
| `0b2189c` | remove google link | Removed the Google Flights deep-link that Task 8 originally emitted alongside the Skyscanner link. `flights_linker` now produces a single URL. `build_task_payloads_test.lua` + `flights_linker_test.lua` updated to match. |
